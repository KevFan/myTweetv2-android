package kevin.mytweet.receivers;
...
public class BootReceiver extends BroadcastReceiver {
  ...
  public void onReceive(Context context, Intent intent) {
    ...
    info("Settings refresh interval value (user-input) " + value + " minute(s)");
    if (NumUtil.isPositiveNumber(value)) {
      interval = Long.parseLong(value) * NUMBER_MILLIS_PER_MINUTE; // minutes to millis
      info("Using refresh interval obtained from settings " + interval);
    }
